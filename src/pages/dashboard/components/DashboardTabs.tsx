...
const DashboardTabs: React.FC<DashboardTabsProps> = ({
  ...
  const seClasses = classes.filter(cls => cls.grade === "SE MME");
  const teClasses = classes.filter(cls => cls.grade === "TE MME");
  const beClasses = classes.filter(cls => cls.grade === "BE MME");

  const AddSubjectCard = ({ grade }: { grade: string }) => (
    <div className="border rounded-lg p-6 text-center hover:bg-muted/10 transition-colors cursor-pointer">
      <Button variant="outline" className="gap-2" onClick={() => {/* Add dialog logic */}}>
